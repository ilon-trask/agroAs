...
import BusinessCatalogItem from "src/pages/MainPage/components/BusinessCatalogItem";
import { getPublicBusiness } from "src/http/requests";
import useBusinessDirection from "src/shared/hook/useBusinessDirection";
import useEnterpriseForm from "src/shared/hook/useEnterpriseForm";
    ...
    >
      ...
      >
        ...
        <TabPanels>
          <TabPanel>
            <Box>
              <MyHeading mb={4}>Розрахуноки прямих витрат</MyHeading>
              <Box
                display={"grid"}
                gridTemplateColumns={[
                  "1fr",
    ...
    >
      ...
      >
        ...
        <TabPanels>
          <TabPanel p={0}>
            <Box>
              <MyHeading mb={4}>Приклади штатних розписів</MyHeading>
              <Box
                display={"grid"}
                gridTemplateColumns={[
                  "1fr",
...
const ObserverWorkerScreen = observer(WorkerScreen);

function MainPage() {
  const { map, business } = useContext(Context);
  useEffect(() => {
    if (!business.publicBusinessPlan[0]) getPublicBusiness(map, business);
  }, []);
  ...
  return (
    <Box>
      ...
      <ObserverWorkerScreen />
      <ObserverMapScreen />
      <ObserverBusinessScreen />
    </Box>
  );
}
